...
Supply-Use Tables (MoSPI, 140 products × 140 industries) are converted via the Product Technology Assumption (PTA; Eurostat 2008). Core EEIO identity:

```
TWF_indirect = W · L · Y     [m³; supply-chain embedded water]
Scarce_TWF   = TWF × WSI     [m³; stress-weighted, WSI from Aqueduct 4.0]
L = (I − A)⁻¹               [Leontief inverse]
```

**W** = diagonal water-intensity matrix (m³/₹ crore, EXIOBASE 3.8 WaterGAP); **Y** = tourism final demand vector. Blue water (groundwater/surface extraction) and green water (rainfed crop ET, agriculture rows only) extracted separately from EXIOBASE F.txt (Hoekstra & Mekonnen 2012). Hawkins-Simon condition ρ(A) < 1 verified for all years.

### 2.2 Tourism Demand Vectors

Base TSA 2015–16 extrapolated via NAS Statement 6.1 real GVA growth × CPI deflator (Temurshoev & Timmer 2011): `Y(t) = TSA_base × [GVA(t)/GVA_base] × [CPI(t)/CPI_base]`. Separate inbound and domestic vectors produced per year.

### 2.3 Scarce and Green Water

Scarce TWF weights blue water by WRI Aqueduct 4.0 sector WSI (agriculture 0.827, manufacturing 0.814, services 0). Green water propagated via EEIO through agriculture rows only — not a proportional allocation.

### 2.4 Direct TWF

Activity-based operational water for four categories:

```
Hotel_m3      = tourist_nights × segment_share × L/room/night
Restaurant_m3 = tourist_days × meals/day × L/meal
Rail_m3       = dom_tourists × modal_share(0.25) × avg_km × L/pkm
Air_m3        = air_pax × tourist_share × L/passenger
```

Direct water is typically < 5% of total TWF. LOW/BASE/HIGH scenarios bracket coefficient uncertainty.

### 2.5 Structural Decomposition Analysis

Two-polar Dietzenbacher–Los (1998): `ΔTWF = W_effect + L_effect + Y_effect`. Each effect uses arithmetic mean of forward and backward polar assignments. Near-cancellation flag raised when any effect exceeds 5×|ΔTWF|.

### 2.6 Monte Carlo Uncertainty

`agr_mult_i ~ LogNormal(μ=0, σ=0.30)`, n = 10,000, seed = 42. Single correlated multiplier across all agricultural rows (conservative upper bound). Spearman ρ² variance decomposition identifies dominant uncertainty source.

### 2.7 Outbound TWF

`Outbound_m3 = tourists × dest_share × avg_stay × (national_WF/365) × 1.5`. The 1.5× tourist multiplier reflects higher per-day consumption than local residents (Hadjikakou et al. 2015). Net balance = Outbound − Inbound (directional indicator only; methods differ across legs).

### 2.8 Validation

Nine automated assertions: IO balance error < 1%; ρ(A) < 1; LOW < BASE < HIGH monotonicity; SDA residual < 0.001%; Scarce/Blue ratio ∈ [0.30, 0.95].
...
**Methodology note — how each column in Table 7 is computed**

| Column | Method | Data source |
|---|---|---|
| Indirect Blue (M m³) | EEIO: W·L·Y restricted to concordance SUT_Product_IDs for this category. Each SUT product contributes once (no double-counting across categories). | EXIOBASE 3.8 F.txt blue rows + MoSPI SUT Leontief inverse + TSA demand vector |
| Green (M m³) | Same EEIO propagation using green WaterGAP coefficients. Non-zero for agriculture-linked categories only; all other sectors carry zero green coefficients. | EXIOBASE 3.8 F.txt "Water Consumption Green" rows |
| Direct (M m³) | Total direct TWF (hotels, restaurants, rail, air) distributed to categories proportionally by tourism demand share. Approximation — direct water is operationally sector-agnostic. | direct_twf_{year}.csv BASE scenario |
| Agr % | Share of this category's indirect blue water originating from Agriculture source group (SUT products 1–29) via Leontief pull: Σ W[i]·L[i,j]·Y[j] where i ∈ agriculture. | indirect_water_{year}_structural.csv grouped by Source_Group = "Agriculture" |
| m³/₹ cr | Indirect Blue ÷ Tourism Demand (₹ crore). Lower = more water-efficient per rupee of tourism spending. 2021-22 values only; see S7d for full time series. | Derived from above columns |
...
| Energy source | EXIOBASE 3.8 `IOT_{year}_ixi/energy/F.txt`; row `"Energy use - Final"` (direct purchased energy inputs per sector). F.txt stores absolute flows (TJ) — normalised by `x.txt` sectoral output (M-EUR) to obtain intensity coefficients. See §2.3 energy methods. Stadler et al. (2018); Rasul et al. (2024). |
...
where **E_final** (140×140 diagonal) contains sector final energy intensities from EXIOBASE 3.8 `"Energy use - Final"` row (MJ/₹ crore, see §2.3 for row selection rationale), **E_fossil** contains emission-relevant intensities from `"Energy use - Emission relevant"` row, **L** is the Leontief inverse, and **Y** (140×1) is the tourism final demand vector. Fossil share = E_fossil·L·Y / E_final·L·Y, capped at 100% (India produces more emission-relevant energy than it domestically consumes in final demand terms in EXIOBASE accounting).
...
Energy intensity coefficients are derived from the EXIOBASE 3.8 energy satellite (`energy/F.txt`) following the approach of Stadler et al. (2018) and the row-selection guidance in Rasul et al. (2024).

**Row selection.** EXIOBASE 3.8 energy/F.txt contains four rows per sector: `Gross`, `Final`, `Net`, and `Emission relevant`. We use:

- **Primary: `Energy use - Final`** — direct purchased/consumed energy inputs per sector. For coal-fired power this equals the coal fuel burned (~30 TJ/M-EUR); for hydropower it equals only operational energy (pumps, controls, ~1 TJ/M-EUR). This is the physically correct intensity measure for EEIO supply-chain analysis. `Gross` was rejected because it double-counts energy before and after transformation steps (Rasul et al. 2024: "gross accounts double count the energy before and after transformation steps, making net energy use accounts more intuitive"). `Net` was also rejected because for electricity generation sectors it counts the electricity *carrier* flowing through the sector rather than the fuel input, producing artificially inflated coefficients (e.g. coal electricity Net/x = 772 TJ/M-EUR vs Final/x = 30 TJ/M-EUR).
- **Secondary: `Energy use - Emission relevant`** — the fossil/emission-relevant subset of energy flows, used to compute the fossil share of indirect energy footprint.

**Normalisation.** F.txt stores absolute energy flows (total TJ consumed by each sector), not intensity coefficients. We convert to intensity by dividing by sectoral output from `x.txt` (M-EUR): `coefficient = F[sector] / x[sector]`. Sectors with output below 0.01% of total India output (~376 M-EUR in 2015, ~502 M-EUR in 2019) are set to zero to avoid noise from sectors with near-zero India production (e.g. tidal, geothermal power).

**Fossil share.** The ratio `Emission relevant / Final` at the India total level exceeds 1.0 (= 1.37 in 2015) because India is a net energy exporter in EXIOBASE accounting — it produces more emission-relevant energy products (coal, oil, gas) than it domestically consumes in final demand terms. The reported fossil share of indirect energy footprint is therefore capped at 100%.

**Unit conversion.** `coefficient [TJ/M-EUR] × 10⁶ [MJ/TJ] × (10/EUR_INR) [M-EUR/crore] = MJ/crore`, where EUR_INR is the annual average exchange rate (RBI reference rates).

**References.** Stadler et al. (2018) EXIOBASE 3, *J. Industrial Ecology* 22(3): 502–515. https://doi.org/10.1111/jiec.12715. Rasul et al. (2024) Energy accounts in EXIOBASE, *J. Industrial Ecology*. https://doi.org/10.1111/jiec.13563.
