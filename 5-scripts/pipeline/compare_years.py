...
# ── Water Cost Coverage: shadow prices and GDP contribution ──────────────────
# Loaded from reference_data.md via config's load_reference_data().
# If config does not yet expose these, fall back to hardcoded defaults that
# match reference_data.md exactly — so the pipeline never crashes on first run.
try:
    from config import WATER_SHADOW_PRICES, TOURISM_GDP_CONTRIBUTION  # type: ignore
except ImportError:
    # Fallback: read directly from reference_data.md using utils loader.
    # This happens when config.py has not yet been updated to expose these sections.
    # Values match reference_data.md § WATER_SHADOW_PRICES and § TOURISM_GDP_CONTRIBUTION.
    WATER_SHADOW_PRICES = {"low": 2.0, "base": 28.0, "high": 95.0}
    # Values from MoT ITS Compendium Table 7.4.1 — Tourism Direct GDP (₹ crore, nominal)
    # 2015 = Third TSA 2015-16 | 2019 = FY2019-20 | 2022 = FY2021-22 (COVID-suppressed)
    TOURISM_GDP_CONTRIBUTION = {"2015": 364668.0, "2019": 541167.0, "2022": 213653.0}
from utils import (
...
def fill_report_template(start_ts: float, steps_req: list,
                          ...
                          log: Logger = None,
                          coverage_df: pd.DataFrame = None) -> Path | None:
    tmpl = Path(__file__).parent / "report_template.md"
    if not tmpl.exists():
        alt = Path(__file__).parent.parent / "templates" / "report_template.md"
        if alt.exists():
            tmpl = alt
        else:
    ...
    # ── Water Cost Coverage tokens ─────────────────────────────────────────────
    if coverage_df is not None and not coverage_df.empty:
        text = _fill_water_coverage_tokens(text, coverage_df, log)
...
# ══════════════════════════════════════════════════════════════════════════════
# WATER COST COVERAGE  &  WATER PRODUCTIVITY
# ══════════════════════════════════════════════════════════════════════════════

def water_cost_coverage(log: Logger = None) -> pd.DataFrame:
    """
    Compute Water Cost Coverage % and Water Productivity for every study year.

    CONCEPT
    ───────
    Coverage % = fraction of tourist spending that water alone represents,
    priced at three shadow-price scenarios from reference_data.md.

        Coverage_pct(scenario) =
            TWF_blue_m3  ×  price_inr_per_m3(scenario)
            ─────────────────────────────────────────── × 100
               Tourism_Demand_crore  ×  10_000_000

    Because water is only ONE resource dimension (energy, land, and food not
    yet quantified), Coverage_pct is a LOWER BOUND on total resource cost.
    This framing lets the paper claim: "Water alone already represents X% of
    tourist spending — future research will determine whether the full resource
    budget exceeds 100%."

    THREE SCENARIOS (prices from reference_data.md § WATER_SHADOW_PRICES)
    ───────────────────────────────────────────────────────────────────────
    low  = ₹2/m³   actual subsidised tariff (CWC 2021)
           → shows how underpriced water is; Coverage will be < 1%
    base = ₹28/m³  economic shadow price (World Bank India Water Economy 2005)
           → headline finding for the paper
    high = ₹95/m³  replacement / depletion cost (NITI Aayog CWMI 2018)
           → upper-bound scenario; most alarming

    SCARCE-WATER VARIANT
    ────────────────────
    Coverage_scarce_base uses Scarce_TWF (WSI-weighted) as numerator.
    This answers: "What fraction is backed by water from ALREADY-STRESSED basins?"
    Agriculture WSI = 0.827, so scarce coverage is consistently below base coverage
    by the ratio (Scarce_TWF / Blue_TWF).

    WATER PRODUCTIVITY
    ──────────────────
    Productivity_inr_per_m3 = (Tourism_GDP_direct_crore × 1e7) / TWF_blue_m3

    Compare to:
      Agriculture ~₹2–8/m³ (Kumar 2005; Planning Commission 2011)
      Manufacturing ~₹40–80/m³ (Annual Survey of Industries + CWC)
    If tourism productivity > agriculture, reallocating water to tourism is
    economically net-positive — but the scarce coverage % shows the social cost.

    OUTPUTS
    ───────
    comparison/water_cost_coverage_all_years.csv  (one row per Year × Segment)
    Columns: Year, Segment, TWF_blue_m3, Scarce_TWF_m3, Demand_crore,
             Price_low, Price_base, Price_high,
             Coverage_low_pct, Coverage_base_pct, Coverage_high_pct,
             Coverage_scarce_base_pct,
             Tourism_GDP_direct_crore, Water_Productivity_inr_per_m3
    """
    log and log.section("WATER COST COVERAGE  &  WATER PRODUCTIVITY")

    prices = WATER_SHADOW_PRICES   # {low, base, high}  ₹/m³
    rows_out = []

    for year in STUDY_YEARS:

        # ── 1. Load TWF ───────────────────────────────────────────────────────
        cat_df   = _safe_csv(DIRS["indirect"] / f"indirect_twf_{year}_by_category.csv")
        dir_df   = _safe_csv(DIRS["direct"]   / f"direct_twf_{year}.csv")
        split_df = _safe_csv(DIRS["indirect"] / f"indirect_twf_{year}_split.csv")
        all_df   = _safe_csv(DIRS["indirect"] / "indirect_twf_all_years.csv")

        # Indirect blue + scarce
        indir_blue_m3   = float(cat_df["Total_Water_m3"].sum()) \
            if not cat_df.empty and "Total_Water_m3" in cat_df.columns else 0.0
        indir_scarce_m3 = float(cat_df["Scarce_m3"].sum()) \
            if not cat_df.empty and "Scarce_m3" in cat_df.columns else 0.0

        # Fall back to all_years summary for scarce if cat has no Scarce_m3 column
        if indir_scarce_m3 == 0 and not all_df.empty:
            r_all = _year_row(all_df, year)
            if r_all is not None:
                indir_scarce_m3 = _col(r_all, "Scarce_TWF_billion_m3") * 1e9

        # Direct TWF (BASE scenario total)
        direct_m3 = 0.0
        if not dir_df.empty:
            for col_name in ("Total_m3", "Total_BASE_m3", "Direct_Total_m3"):
                if col_name in dir_df.columns:
                    base_rows = dir_df[dir_df.get("Scenario", pd.Series(dtype=str)) == "BASE"] \
                        if "Scenario" in dir_df.columns else dir_df
                    direct_m3 = float(base_rows[col_name].sum())
                    break

        total_blue_m3   = indir_blue_m3  + direct_m3
        # Direct water is on-site municipal supply — not WSI-weighted (services WSI = 0).
        # Scarce TWF covers indirect supply-chain blue water only.
        total_scarce_m3 = indir_scarce_m3

        # ── 2. Load demand (tourist spending) ─────────────────────────────────
        # Y_tourism vectors: sum of Tourism_Demand_crore = total spending in ₹ crore
        def _demand_crore(path) -> float:
            df = _safe_csv(path)
            if df.empty: return 0.0
            for col_name in ("Tourism_Demand_crore", "Demand_crore", "Value"):
                if col_name in df.columns:
                    return float(df[col_name].sum())
            return 0.0

        demand_total_cr = _demand_crore(DIRS["demand"] / f"Y_tourism_{year}.csv")
        demand_inb_cr   = _demand_crore(DIRS["demand"] / f"Y_tourism_{year}_inbound.csv")
        demand_dom_cr   = _demand_crore(DIRS["demand"] / f"Y_tourism_{year}_domestic.csv")

        # ── 3. Load inbound / domestic TWF split ─────────────────────────────
        inb_twf_m3 = dom_twf_m3 = 0.0
        if not split_df.empty and {"Type", "TWF_m3"}.issubset(split_df.columns):
            inb_r = split_df[split_df["Type"] == "Inbound"]
            dom_r = split_df[split_df["Type"] == "Domestic"]
            if not inb_r.empty: inb_twf_m3 = float(inb_r["TWF_m3"].iloc[0])
            if not dom_r.empty: dom_twf_m3 = float(dom_r["TWF_m3"].iloc[0])
        if inb_twf_m3 == 0 and total_blue_m3 > 0:
            # Proportional fallback: split by inbound/domestic tourist-days
            act = ACTIVITY_DATA.get(year, {})
            dom_days = act.get("domestic_tourists_M", 0) * act.get("avg_stay_days_dom", 3.5)
            inb_days = act.get("inbound_tourists_M",  0) * act.get("avg_stay_days_inb", 8.0)
            all_days = dom_days + inb_days
            inb_twf_m3 = total_blue_m3 * (inb_days / all_days) if all_days else 0
            dom_twf_m3 = total_blue_m3 * (dom_days / all_days) if all_days else 0

        # ── 4. Scarce water pro-rata for inbound / domestic ───────────────────
        scarce_ratio = total_scarce_m3 / total_blue_m3 if total_blue_m3 > 0 else 0.0
        inb_scarce_m3 = inb_twf_m3 * scarce_ratio
        dom_scarce_m3 = dom_twf_m3 * scarce_ratio

        # ── 5. Tourism GDP for water productivity ─────────────────────────────
        gdp_cr = TOURISM_GDP_CONTRIBUTION.get(year, 0.0)

        # ── 6. Core formulas ──────────────────────────────────────────────────
        def _coverage(twf_m3: float, demand_cr: float, price: float) -> float:
            """Coverage % = (TWF_m3 × price_inr_m3) / (demand_cr × 1e7) × 100"""
            if demand_cr <= 0 or twf_m3 <= 0:
                return 0.0
            return round(100.0 * (twf_m3 * price) / (demand_cr * 1e7), 4)

        def _productivity(gdp_cr: float, twf_m3: float) -> float:
            """₹ per m³ = (GDP_crore × 1e7) / TWF_m3"""
            if twf_m3 <= 0 or gdp_cr <= 0:
                return 0.0
            return round((gdp_cr * 1e7) / twf_m3, 2)

        p_low  = prices.get("low",  2.0)
        p_base = prices.get("base", 28.0)
        p_high = prices.get("high", 95.0)

        segments = [
            # (name, blue_m3, scarce_m3, demand_cr, include_productivity)
            ("Combined", total_blue_m3,  total_scarce_m3, demand_total_cr, True),
            ("Inbound",  inb_twf_m3,    inb_scarce_m3,   demand_inb_cr,   False),
            ("Domestic", dom_twf_m3,    dom_scarce_m3,   demand_dom_cr,   False),
        ]

        for seg_name, twf_m3, scarce_m3, demand_cr, do_prod in segments:
            rows_out.append({
                "Year":                      year,
                "Segment":                   seg_name,
                "TWF_blue_m3":               round(twf_m3),
                "Scarce_TWF_m3":             round(scarce_m3),
                "Demand_crore":              round(demand_cr, 2),
                "Price_low_inr_m3":          p_low,
                "Price_base_inr_m3":         p_base,
                "Price_high_inr_m3":         p_high,
                "Coverage_low_pct":          _coverage(twf_m3, demand_cr, p_low),
                "Coverage_base_pct":         _coverage(twf_m3, demand_cr, p_base),
                "Coverage_high_pct":         _coverage(twf_m3, demand_cr, p_high),
                "Coverage_scarce_base_pct":  _coverage(scarce_m3, demand_cr, p_base),
                "Tourism_GDP_direct_crore":  gdp_cr if do_prod else 0.0,
                "Water_Productivity_inr_per_m3": _productivity(gdp_cr, twf_m3) if do_prod else 0.0,
            })

        # ── 7. Log summary ────────────────────────────────────────────────────
        comb = rows_out[-3]  # Combined row (always first of the three segments)
        msg = (
            f"  {year}  Combined TWF = {total_blue_m3/1e9:.4f} bn m³  |  "
            f"Demand = ₹{demand_total_cr:,.0f} cr  |  "
            f"Coverage: low={comb['Coverage_low_pct']:.2f}%  "
            f"base={comb['Coverage_base_pct']:.2f}%  "
            f"high={comb['Coverage_high_pct']:.2f}%  "
            f"scarce(base)={comb['Coverage_scarce_base_pct']:.2f}%"
        )
        log.ok(msg.strip()) if log else print(msg)

        if gdp_cr > 0:
            prod_msg = (
                f"  {year}  Water Productivity = ₹{comb['Water_Productivity_inr_per_m3']:,.0f}/m³  "
                f"(GDP ₹{gdp_cr:,.0f} cr ÷ {total_blue_m3/1e9:.4f} bn m³)"
            )
            log.info(prod_msg.strip()) if log else print(prod_msg)

    df = pd.DataFrame(rows_out)

    # Cross-year trend for headline metric
    comb_df = df[df["Segment"] == "Combined"]
    compare_across_years(
        {r["Year"]: r["Coverage_base_pct"] for _, r in comb_df.iterrows()},
        "Water Cost Coverage % — base shadow price (₹28/m³) — Combined",
        unit="%", decimals=2, log=log,
    )
    compare_across_years(
        {r["Year"]: r["Water_Productivity_inr_per_m3"] for _, r in comb_df.iterrows()},
        "Water Productivity (₹ per m³) — Combined",
        unit="", decimals=0, log=log,
    )

    return df


def _fill_water_coverage_tokens(text: str, coverage_df: pd.DataFrame,
                                 log: Logger = None) -> str:
    """
    Fill all {{WATER_COST_*}} and {{WATER_PROD_*}} template tokens from the
    coverage DataFrame produced by water_cost_coverage().

    Tokens filled
    ─────────────
    {{WATER_COST_COVERAGE_ROWS}}        — main table rows (Year × Segment × 3 scenarios)
    {{WATER_COST_COVERAGE_BASE_2015}}   — base coverage %, 2015, Combined
    {{WATER_COST_COVERAGE_BASE_2019}}
    {{WATER_COST_COVERAGE_BASE_2022}}
    {{WATER_COST_COVERAGE_HIGH_2022}}   — high scenario, 2022
    {{WATER_COST_COVERAGE_SCARCE_2022}} — scarce-water base, 2022
    {{WATER_PROD_2015}}                 — water productivity ₹/m³
    {{WATER_PROD_2019}}
    {{WATER_PROD_2022}}
    {{WATER_COST_PRICE_LOW}}            — price assumption labels for footnotes
    {{WATER_COST_PRICE_BASE}}
    {{WATER_COST_PRICE_HIGH}}
    """
    if coverage_df.empty:
        return text

    prices = WATER_SHADOW_PRICES

    # Build main table rows: one row per (Year, Segment)
    table_rows = ""
    for _, r in coverage_df.iterrows():
        ...
        dem  = r["Demand_crore"]
        twf  = r["TWF_blue_m3"]
        scar = r["Scarce_TWF_m3"]
        table_rows += (
            f"| {yr} | {seg} "
            f"| {int(twf/1e6):,}M | ₹{dem:,.0f} "
            f"| {r['Coverage_low_pct']:.2f}% "
            f"| **{r['Coverage_base_pct']:.2f}%** "
            f"| {r['Coverage_high_pct']:.2f}% "
            f"| {r['Coverage_scarce_base_pct']:.2f}% "
            f"| {int(r['Water_Productivity_inr_per_m3']):,} |\\n"
        )
    text = text.replace("{{WATER_COST_COVERAGE_ROWS}}", table_rows or "| - |\n")

    # Scalar tokens per year
    for yr in STUDY_YEARS:
        comb = coverage_df[(coverage_df["Year"] == yr) & (coverage_df["Segment"] == "Combined")]
        inb  = coverage_df[(coverage_df["Year"] == yr) & (coverage_df["Segment"] == "Inbound")]
        dom  = coverage_df[(coverage_df["Year"] == yr) & (coverage_df["Segment"] == "Domestic")]

        def _v(df, col, fmt=".2f"):
            return f"{float(df.iloc[0][col]):{fmt}}" if not df.empty else "-"

        text = text.replace(f"{{{{WATER_COST_COVERAGE_LOW_{yr}}}}}",    _v(comb, "Coverage_low_pct"))
        text = text.replace(f"{{{{WATER_COST_COVERAGE_BASE_{yr}}}}}",   _v(comb, "Coverage_base_pct"))
        text = text.replace(f"{{{{WATER_COST_COVERAGE_HIGH_{yr}}}}}",   _v(comb, "Coverage_high_pct"))
        text = text.replace(f"{{{{WATER_COST_COVERAGE_SCARCE_{yr}}}}}", _v(comb, "Coverage_scarce_base_pct"))
        text = text.replace(f"{{{{WATER_PROD_{yr}}}}}",
                            f"{int(float(comb.iloc[0]['Water_Productivity_inr_per_m3'])):,}" if not comb.empty else "-")
        text = text.replace(f"{{{{WATER_COST_COVERAGE_BASE_INB_{yr}}}}}", _v(inb, "Coverage_base_pct"))
        text = text.replace(f"{{{{WATER_COST_COVERAGE_BASE_DOM_{yr}}}}}", _v(dom, "Coverage_base_pct"))

    # Price labels for footnotes
    text = text.replace("{{WATER_COST_PRICE_LOW}}",  f"₹{prices.get('low',2.0):.0f}/m³")
    text = text.replace("{{WATER_COST_PRICE_BASE}}", f"₹{prices.get('base',28.0):.0f}/m³")
    text = text.replace("{{WATER_COST_PRICE_HIGH}}", f"₹{prices.get('high',95.0):.0f}/m³")
...
def run(start_ts: float = None, steps_req: list = None,
    ...
    with Logger(SCRIPT_NAME, DIRS["logs"]) as log:
        ...
        coverage_df          = water_cost_coverage(log)           # ← NEW
        ...
        if not coverage_df.empty:                                                        # ← NEW
            save_csv(coverage_df,  DIRS["comparison"] / "water_cost_coverage_all_years.csv",
                     "Water cost coverage", log=log)
