...
def _build_water_by_source_rows() -> str:
    ...
    Uses indirect_twf_{last_yr}_origin.csv for blue volumes and
    water_coefficients_{last_yr}.csv for green coefficients.
    """
    last_yr   = STUDY_YEARS[-1]
    orig_df   = _load_csv_cached(DIRS["indirect"] / f"indirect_twf_{last_yr}_origin.csv")
    blue_by_grp: dict  = {}
    green_by_grp: dict = {}
...
def _fill_narrative_placeholders(text: str, first_yr: str, last_yr: str,
    ...
    #
    # Sources:
    #   Ind. Blue   = TWF_m3    in indirect_twf_{yr}_split.csv   ← blue EEIO W·L·Y_seg
    ...
    def _build_seg_rows():
        ...
        for yr in STUDY_YEARS:
            ...
            sp = _safe_csv(DIRS["indirect"] / f"indirect_twf_{yr}_split.csv")
            ...
            if not sp.empty and "Type" in sp.columns:
                ...
                _ag = _load_csv_cached(DIRS["indirect"] / "indirect_twf_all_years.csv")
            ...
            rows += (
                f"| {yr} | Inbound  | {inb_dem_cr:,.0f} "
                f"| {inb_blue_bn:.4f} | {inb_grn_bn:.4f}{g_flag} | {inb_indir_bn:.4f} "
                f"| {inb_direct_bn:.4f} | **{inb_total_bn:.4f}** "
                f"| {inb_blue_lpd_s} | {inb_lpd_s} | {sp_i} | {ra_s} | {sr_s} | {re_s} |\n"
            )
            rows += (
                f"| {yr} | Domestic | {dom_dem_cr:,.0f} "
                f"| {dom_blue_bn:.4f} | {dom_grn_bn:.4f}{g_flag} | {dom_indir_bn:.4f} "
                f"| {dom_direct_bn:.4f} | **{dom_total_bn:.4f}** "
    ...
    # ── SECTOR_DECOMP_CROSS_YEAR_ROWS (Table 24 / Main Table 7) ─────────────
    # Reads indirect_twf_{yr}_sector_decomp.csv for each year.
    # Shows top-8 TSA categories (by avg Total_m3 across years) × 3 years.
    # Columns: Rank | TSA Category | Year | Demand (₹ cr) | Indirect (bn m³) |
    #          Direct (bn m³) | Total (bn m³) | Direct % | Agr % | Elec % | Petro %
    #
    # Falls back gracefully if file absent (pre-pipeline or old run).
    ...
    for yr in STUDY_YEARS:
        _df = _safe_csv(DIRS["indirect"] / f"indirect_twf_{yr}_sector_decomp.csv")
        if not _df.empty:
            _df["_yr"] = yr
    ...
    _direct_by_cat: dict[str, list[float]] = {}  # category -> [direct_pct across years]

    if _decomp_frames:
        # Column detection across all years combined
        ...
        if _ncol and _tcol:
            # NEW: Year-first ordering — for each year show categories ranked highest→lowest
            # Use top-8 categories per year (union across years, ranked within each year)
            _top_cats_union: set = set()
            for yr in STUDY_YEARS:
                _df = _decomp_frames.get(yr)
                if _df is not None and _ncol in _df.columns and _tcol in _df.columns:
                    _yr_top = list(_df.nlargest(8, _tcol)[_ncol])
                    _top_cats_union.update(_yr_top)
            _top8 = sorted(_top_cats_union)  # stable name list for scalar tokens below
            ...
            for yr in STUDY_YEARS:
                ...
                if _df is None:
                    ...
                    continue
                # Sort this year's categories highest → lowest by Total_m3
                _yr_sorted = (
                    _df[_df[_ncol].isin(_top_cats_union)]
                ...
                for rank_yr, (_, row) in enumerate(_yr_sorted.iterrows(), 1):
                    ...
                    decomp_rows += (
                        f"| {yr} | {rank_yr} | {cat} | {dem_cr:,.0f} "
                        f"| {ind_bn:.4f} | {dir_bn:.4f} | {tot_bn:.4f} "
                        f"| {dir_pct:.1f}% | {agr_pct:.1f}% | {elec_pct:.1f}% | {petro_pct:.1f}% |\n"
                    )
    else:
        decomp_rows = "| — | sector_decomp CSV not found — re-run calculate_indirect_twf.py | — | — | — | — | — | — | — | — | — |\n"
    ...
    # PMKSY impact on top category: top_cat indirect × agr_pct × 0.20 × elasticity(0.71)
    try:
    ...
    #          Intensity Blue L/day | Intensity Blue+Green L/day | Δ
    _mt1_base = None
    mt1_rows  = ""
    _ind_all_mt1 = _load_csv_cached(DIRS["indirect"] / "indirect_twf_all_years.csv")
    ...
    for yr in STUDY_YEARS:
        ...
        mc_ci     = f"{mc_p5:.4f}–{mc_p95:.4f}" if mc_p5 > 0 else "—"
        mt1_rows += (
            f"| {yr} | {blue_bn:.4f} | {green_bn:.4f} | {bg_bn:.4f} "
            f"| {scar_bn:.4f} | {dir_bn:.4f} | **{tot_blue_bn:.4f}** "
            f"| {mc_p5:.4f} | {mc_p95:.4f} "
            f"| {int_blue} | {int_bg} | {delta_mt1} |\n"
        )
    ...
    outb_dest_df  = _load_csv_cached(outb_dest_dir / "outbound_twf_by_dest.csv")
    outb_dest_rows = ""
    if not outb_dest_df.empty:
        ...
        for _, r in outb_dest_df.iterrows():
            ...
            outb_dest_rows += (
                ...
                f"| {_dv('Avg_stay_days','.1')} | {_dv('Outbound_m3',',')} "
                f"| {_dv('Scarce_m3',',')} |\n"
            )
    ...
    origin_top10_rows = ""
    orig_df = _safe_csv(DIRS["indirect"] / f"indirect_twf_{last_yr}_origin.csv")
    if not orig_df.empty and "Water_m3" in orig_df.columns:
        tot_w = orig_df["Water_m3"].sum()
        nm_col_orig = next((c for c in ("Product_Name","Sector_Name","Source_Name") if c in orig_df.columns), None)
        ...
        for rank, (_, r) in enumerate(top10_orig.iterrows(), 1):
            ...
            origin_top10_rows += (
                f"| {rank} | {nm} | {grp} | {int(w):,} | {pct:.1f}% "
                f"| {wsi_w:.3f} | {int(sc_m3):,} | {sc_pct} |\n"
            )
